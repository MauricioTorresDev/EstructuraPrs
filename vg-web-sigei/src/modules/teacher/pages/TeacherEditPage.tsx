...
export function TeacherEditPage() {
     ...
     const loadData = async () => {
          ...
          try {
               setLoading(true);
               // First fetch the assignment to get the institution ID
               const assignmentResponse = await teacherAssignmentService.getById(id);
               const assignment = assignmentResponse.data;

               // Then fetch other data, including the specific institution
     ...
     const updateSchedule = (index: number, field: string, value: string) => {
          if (field === "startTime" || field === "endTime") {
               if (value && (value < "08:00" || value > "12:30")) {
                    showErrorAlert(
                         "Horario inválido",
                         "El horario debe estar entre las 08:00 AM y las 12:30 PM"
                    );
                    return;
               }
          }
